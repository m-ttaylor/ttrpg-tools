...
const NameEntryItem = ({ name, hint, existingValue }: { name: string, hint: string, existingValue: string }) => (
    ...
    <textarea className="lg:w-2/3 rounded text-3xl bg-white/50 lg:text-4xl text-slate-600 font-bold p-2"
      // type="text"
      rows={1}
      id={name}
      name={name}
...
const DescriptionEntryItem = ({name, hint, existingValue}: {name: string, hint: string, existingValue: string}) => (
  <div className="">
    <textarea
      className="rounded w-full resize-y bg-white/50 text-xl italic p-2"
      // type="text"
      rows={1}
      id={name}
      name={name}
...
const RecipeEditForm = ({ recipe, updateRecipe }: Props) => {
  ...
  return (
    ...
    {/*     
          ...
          {/* <div className="relative flex-wrap mt-10"> */}
            {/* <span className="flex"> */}
              <NameEntryItem name="name" hint="" existingValue={recipe.name} />
            {/* </span> */}
            <div className="inline-block text-sm px-4 py-2 leading-none border rounded text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 mt-6 lg:mt-0">
              <Link
                // className="absolute right-5 my-2 py-2 px-5 ml-12 mr-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                href={`/recipes/${recipe._id}`}
              >
                go back
              </Link>
